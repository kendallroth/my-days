 ...
 */
const validateEnum = <T>(enumeration: any, value: string | null, defaultValue: T): [boolean, T] => {
  if (!value) return [false, defaultValue];

  return isInEnum(enumeration, value) ? [true, value as unknown as T] : [false, defaultValue];
};
